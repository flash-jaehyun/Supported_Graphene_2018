"""BCC Iron Lattice Cell Optimization."""
...
tree_level_values = [
    [400, 600, 800],
    ["222", "444", "666"],
    np.linspace(2.6, 3.2, num=100).tolist(),
    ]
...
for step in range(len(step_dir_names)):
    ...
    if not os.path.isfile(files_placed_file):
        ...
        for job_i in JobsAn.job_var_lst:
            ...
            var = "kpoints"
            if var in job_i_params:
                kpts = job_i_params[var]
                kpts_tuple = (int(kpts[0]), int(kpts[1]), int(kpts[2]))

                qe_par.update_params({"kpts": kpts_tuple})

            qe_par.write_params(path=path_i)
            #__|

            #| - Atoms Object
            atoms_i = bulk("Fe", "bcc", a=job_i_params["lattice-parameter"])
            atoms_i = atoms_i.repeat(2)
